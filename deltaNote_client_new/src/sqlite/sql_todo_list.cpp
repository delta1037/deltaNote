...
void SqlTodoList::turn_to_struct(const SqlRetList &sql_ret_list, TodoList &ret_struct_list) {
    for(auto ret_it : sql_ret_list){
        ret_struct_list.emplace_back(
                ...
                (ret_it)["data"]);
        d_sql_debug("create_key:%s, edit_key:%s, op_type:%s, is_check:%s, data:%s",
...
int SqlTodoList::sel(const std::string &create_key, TodoList &ret_list, ErrorCode &error_code) {
    static const std::string sel_sql =
            "SELECT create_key, edit_key, op_type, is_check, data from %Q WHERE create_key == %Q";
    SqlRetList sql_ret_list;
    int ret = sql_base->exec(
            sqlite3_mprintf(
                    sel_sql.c_str(),
...
// 修改数据（根据创建时间+操作类型）
int SqlTodoList::alt(const std::string &create_key, OpType op_type, const std::string &edit_key, IsCheck is_check, const std::string &data, ErrorCode &error_code) {
    static const std::string alt_sql =
            "UPDATE %Q SET edit_key = %Q, is_check = %Q, data = %Q "\
