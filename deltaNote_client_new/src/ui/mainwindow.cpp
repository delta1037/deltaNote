...
MainWindow::~MainWindow()
{
    ...
    delta_note_info("deltaNote close, save all todo - size:%d", listBuffer.size());
...
void MainWindow::updateMessage(){
    ...
    delta_note_debug("send massage status code : %d", ret);
#endif
#ifdef WINDOW_CLIENT
...
void MainWindow::checkUpdate(){
    ...
    if(status == ConnectError){
        ...
        delta_note_debug("%s", "update connect server error");
        return;
    }else if(status == AlreadyLatest){
        MessagesBox::info(this, UPDATE_ALREADY_NEW);
        delta_note_debug("%s", "update dont need");
        return;
    }
    ...
    if(ret == QDialog::Rejected){
        ...
        delta_note_debug("%s", "update fail");
    }else if(ret == QDialog::Accepted){
        delta_note_debug("%s", "get update file success");
        ...
        delta_note_info("update program:%s", updateProgram.toLatin1().data());
        if(deltaNote->waitForStarted() == true){
            delta_note_debug("%s", "start success");
            qApp->quit();
        }else{
            delta_note_debug("%s", "start fail");
        }
    }
}
...
void MainWindow::addTodoClicked(){
    ...
    if(0 == ui->ToDoListWin->count()){
        delta_note_debug("%s", "todo list window is null, add new todo");
        addNewTodo();
        return;
    }
...
void MainWindow::dataAudoSave(){
    // 保存设置
    if(settingChange){
        delta_note_debug("linux device setting auto save");
        DataCtrl dataCtrl;
        dataCtrl.setAllSetting();
        settingChange = false;
    }
    ...
    if(ListControl::todoListChange){
        delta_note_debug("linux device data auto save");
        std::vector<SocketMsgOpPack> listBuffer;
        for(auto &it : ListControl::getDoneList()){
            listBuffer.push_back(SocketMsgOpPack(it));
        }
