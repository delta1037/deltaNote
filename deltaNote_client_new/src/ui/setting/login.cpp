...
login::login(QWidget *parent, SettingCtrl *setting_ctrl, SyncData *sync_data) :
        QDialog(parent),
        ui(new Ui::login),
...
void login::paintEvent(QPaintEvent *event)
{
    ...
    painter.fillRect(this->rect(), QColor(icon_color.red(), icon_color.green(), icon_color.blue(), 255 - m_setting_ctrl->get_int(SETTING_TRAN_POS)));
    //setStyleSheet("background-color:transparent");
...
void login::doLogin(){
    ...
    if(nullptr == QS_server_port){
        ...
        //NetStatus status = loginCtrl.loginToServer();
    /*
