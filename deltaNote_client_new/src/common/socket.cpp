...
bool SocketClient::initSocket(char *initServerIP, int initServerPort){
    ...
    if(clientSocketFd == INVALID_SOCKET){
        network_error("%s", "Client socket init failed")
        return false;
    }
    ...
    if(ret == SOCKET_ERROR){
        network_error("%s", "connect server failed")
        return false;
    }
    return true;
}
...
bool SocketClient::sendMsg(void *buf, size_t size) {
    ...
    while(size > 0){
        ...
        if(sendSize <= 0){
            network_info("%s", "client socket closed, stop send")
            close(clientSocketFd);
            clientSocketFd = -1;
            return false;
        }
        ...
        if(size != 0 || (size == 0 && bigDataFlag == true)){
            bigDataFlag = true;
            network_info("big data sending : %d / %d", (int)(needSendSize - size), (int)needSendSize)
        }
    }
    return true;
}
...
bool SocketClient::recvMsg(void *buf, size_t size) {
    ...
    while(size > 0){
        ...
        if(recvSize <= 0){
            ...
            network_info("%s", "client socket closed, stop receive")
            close(clientSocketFd);
            clientSocketFd = -1;
            return false;
        }
        ...
        if(size != 0 || (size == 0 && bigDataFlag == true)){
            bigDataFlag = true;
            network_info("big data receiving : %d / %d", (int)(needRecvSize - size), (int)needRecvSize)
        }
    }
    return true;
}
