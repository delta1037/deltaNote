...
bool CheckUpdate::createNewFile(){
    ...
    LogCtrl::debug("downloadFile:%s", downloadFile.toLatin1().data());

    QDir file(downloadPath);
    if(!file.exists()){
        LogCtrl::debug("create download path");
        file.mkpath(downloadPath);
    }

    QFileInfo fileInfo(downloadFile);
    if(fileInfo.isFile()){
        LogCtrl::debug("delete already download file");
...
void CheckUpdate::getUpdateFile(){
    if(!createNewFile()){
        LogCtrl::debug("create new file error");
        return;
    }

    LogCtrl::debug("start download:%s", updateLink.toLatin1().data());

    //开始请求 下载文件
    QUrl serviceUrl = QUrl(updateLink);

    updateManager = new QNetworkAccessManager(this);
    if(updateManager->networkAccessible() == QNetworkAccessManager::Accessible){
        updateReply = updateManager->get(QNetworkRequest(serviceUrl));
        //if(updateReply != nullptr)
        LogCtrl::debug("update reply : %p", updateReply);
...
void CheckUpdate::processError(QNetworkReply::NetworkError error){
    LogCtrl::debug("error code : %d", error);
}
...
void CheckUpdate::fileDowloadFinished(){
    ...
    LogCtrl::debug("download finish");
}

void CheckUpdate::updateDownloadProgress(qint64 recv, qint64 total){
    LogCtrl::debug("recv:%d total:%d", recv, total);
