...
bool Sqlite::createTable(const char *databaseName, const char *sql){
    ...
    CHECK(ret, SQLITE_ERROR, {LogCtrl::error("OPEN DATABASE SQL error\n"); return false;});
...
bool Sqlite::loadSettingValue(SettingPair &settingPair){
    ...
    CHECK(ret, SQLITE_ERROR, {LogCtrl::error("SQL_USER_SETTING_TABLE_SELECT SQL error : %s\n", zErrMsg); return false;});
    ...
    if(SettingsCallBackValue.size() == 1 && strlen(SettingsCallBackValue[0].value) > 0){
        ...
        if(SettingsCallBackValue.size() == 1 && strlen(SettingsCallBackValue[0].value) == 0){
            LogCtrl::warn("setting : %s is null", settingPair.name);
        }else if(SettingsCallBackValue.size() > 1){
            LogCtrl::warn("setting : %s is too many, clean all", settingPair.name);
            delSetting(SettingsCallBackValue[0]);
        }else{
            LogCtrl::warn("no such setting : %s", settingPair.name);
        }
        return false;
    }
}
...
bool Sqlite::saveSettingValue(SettingPair &settingPair){
    ...
    if(strlen(settingPair.value) == 0){
        ...
        CHECK(ret, SQLITE_ERROR, {LogCtrl::error("SQL_USER_SETTING_TABLE_INSERT SQL error : %s\n", zErrMsg); return false;})
    }else{
...
bool Sqlite::alterSettingValue(SettingPair &settingPair){
    ...
    CHECK(ret, SQLITE_ERROR, {LogCtrl::error("SQL_USER_SETTING_TABLE_UPDATE SQL error : %s\n", zErrMsg); return false;})
    return true;
}
...
bool Sqlite::delSetting(SettingPair &settingPair){
    ...
    CHECK(ret, SQLITE_ERROR, {LogCtrl::error("SQL_USER_SETTING_TABLE_DELETE SQL error : %s\n", zErrMsg); return false;})
    return true;
}
