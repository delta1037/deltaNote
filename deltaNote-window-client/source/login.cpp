...
void login::doLogin(){
    ...
    if(nullptr == QS_server_port){
        ...
        if (control.getState() == LoginSuccess) {
            accept();
        } else if(control.getState() == LoginPasswdError) {
            ui->password->clear();
            ui->password->setFocus();
            MessagesBox::warn(this, LOGIN_PASSWORD_ERROR);
        } else if(control.getState() == LoginUserNotExits) {
            ui->username->clear();
            ui->password->clear();
            ui->username->setFocus();
            MessagesBox::warn(this, LOGIN_USER_N_EXITS);
        } else if(control.getState() == LoginUndefinedError) {
            ui->username->clear();
            ui->password->clear();
            ui->username->setFocus();
